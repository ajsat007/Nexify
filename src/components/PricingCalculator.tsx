...
export default function PricingCalculator() {
  ...
  return (
    <div className="bg-white rounded-2xl border border-surface-200 overflow-hidden shadow-xl">
      <div className="p-6 bg-gradient-to-r from-primary-500/5 to-accent-500/5 border-b border-surface-200">
        ...
        <p className="text-sm text-surface-600">Select options to get an instant AI-generated estimate</p>
      </div>

      <div className="p-6 space-y-8">
        {/* Service Type */}
        <div>
          <h3 className="text-sm font-semibold text-surface-700 mb-3">1. What do you need?</h3>
              ...
              <button key={s.id} onClick={() => { setService(s.id); setSelectedFeatures([]) }}
                className={`flex items-center gap-2 px-4 py-3 rounded-xl border text-sm font-medium transition-all ${
                  service === s.id
                    ...
                    : 'border-surface-200 text-surface-800 hover:border-surface-300'
                }`}>
        ...
        {/* Complexity */}
        <div>
          <h3 className="text-sm font-semibold text-surface-700 mb-3">2. How complex is your project?</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {complexityMultipliers.map(c => (
              <button key={c.id} onClick={() => setComplexity(c.id)}
                ...
                <div className="text-xs text-surface-500 mt-0.5">{c.desc}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Features */}
        <div>
          <h3 className="text-sm font-semibold text-surface-700 mb-3">3. Add features (optional)</h3>
        ...
        {/* Results */}
        <div className="bg-surface-900 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold">Estimated Project Cost</h3>
            <button onClick={reset} className="text-xs text-surface-500 hover:text-white flex items-center gap-1 transition-all">
              <RefreshCw size={12} /> Reset
            </button>
          </div>
          <div className="grid sm:grid-cols-3 gap-4 mb-4">
            <div className="text-center p-4 bg-white/5 rounded-xl">
              <div className="text-xs text-surface-500 mb-1">Base Price</div>
              <div className="text-xl font-heading font-bold">{(baseCost / 100000).toFixed(1)}L</div>
              <div className="text-xs text-surface-500">{currentService.name} · {currentComplexity.label}</div>
            </div>
            <div className="text-center p-4 bg-white/5 rounded-xl">
              <div className="text-xs text-surface-500 mb-1">Features</div>
              <div className="text-xl font-heading font-bold">+{(featuresCost / 1000).toFixed(0)}K</div>
              <div className="text-xs text-surface-500">{selectedFeatures.length} selected</div>
            </div>
          ...
          <a href="/contact" className="w-full inline-flex items-center justify-center gap-2 py-3 rounded-xl bg-white text-surface-900 font-semibold hover:bg-surface-100 transition-all text-sm">
            Get Exact Quote <ArrowRight size={16} />
          </a>
          <p className="text-xs text-surface-600 text-center mt-2">This is an AI estimate. Final pricing determined during scoping.</p>
        </div>
      </div>
    </div>
  )
