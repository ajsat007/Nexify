...
export function AIAgentWidget() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages(p => [...p, { role: 'ai', text: data.response || 'Sorry, I encountered an error.' }])
    } catch {
      ...
      {open && (
        <div className="absolute bottom-16 right-0 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-neutral-200 overflow-hidden mb-2 animate-slide-up" role="dialog" aria-label="Nexify AI Chat">
          ...
          <div className="p-3 border-t border-neutral-200">
            <div className="flex gap-2">
              <input className="flex-1 px-3 py-2 rounded-xl border border-neutral-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30" placeholder="Ask about Nexify..." value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} aria-label="Chat message" />
              <button onClick={send} disabled={loading || !input.trim()} className="w-9 h-9 rounded-xl bg-primary-500 text-white flex items-center justify-center hover:bg-primary-600 disabled:opacity-50 transition-all" aria-label="Send message"><Send size={14} /></button>
            </div>
            <p className="text-[10px] text-neutral-500 mt-1.5 flex items-center gap-1"><Zap size={10} /> Nexify AI — AI-powered assistant</p>
          </div>
        </div>
      )}
      <button onClick={() => setOpen(!open)} className={`w-14 h-14 rounded-2xl gradient-bg text-white shadow-xl hover:shadow-2xl transition-all flex items-center justify-center ${open ? 'rotate-45' : ''}`} aria-label={open ? 'Close chat' : 'Open chat'}>
