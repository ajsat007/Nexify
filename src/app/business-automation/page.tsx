      ...
      <section className="relative pt-32 pb-20 gradient-bg overflow-hidden">
        <div className="section-container relative">
          <div className="max-w-3xl">
            ...
            <p className="text-xl text-surface-400 max-w-2xl">Every business process fully automated. From lead to delivery — zero manual intervention.</p>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding bg-white">
        <div className="section-container">
          <div className="grid md:grid-cols-2 gap-4">
            {automations.map((a, i) => (
              <div key={i} className="bg-surface-50 rounded-2xl p-6 border border-surface-200 hover:shadow-lg hover:border-primary-500/20 transition-all">
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-semibold text-surface-900">{a.name}</h3>
                  <div className="flex items-center gap-1 text-xs text-surface-800"><RefreshCw size={12} /> {a.time}</div>
                </div>
                <div className="text-xs text-surface-800 mb-2">Trigger: {a.trigger}</div>
