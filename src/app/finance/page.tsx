      ...
      <section className="relative pt-32 pb-20 gradient-bg overflow-hidden">
        <div className="section-container relative">
          <div className="max-w-3xl">
            ...
            <p className="text-xl text-surface-400 max-w-2xl">Zero human labor = 95%+ profit margins. All projections are conservative estimates.</p>
          </div>
        </div>
      </section>
        ...
        <div className="section-container">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {[
              ...
              <div key={i} className="text-center p-6 bg-surface-50 rounded-2xl border border-surface-200">
                ...
                <div className="text-xs text-surface-800 mt-0.5">{s.sub}</div>
              </div>
            ))}
          </div>
            ...
            <div className="bg-surface-50 rounded-2xl p-6 border border-surface-200">
                ...
                {monthlyExpenses.map((e, i) => (
                  <div key={i}>
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span className="text-surface-700">{e.category}</span>
                      <span className="font-semibold text-surface-900">{e.amount.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="w-full bg-white rounded-full h-2.5 border border-surface-200">
              ...
              <div className="mt-6 p-4 bg-white rounded-xl border border-surface-200">
                <div className="flex items-center justify-between text-sm">
                  <span className="font-semibold text-surface-900">Total Monthly Burn</span>
                  <span className="font-semibold text-surface-900">{totalMonthly.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
                ...
                {projections.map((p, i) => (
                  <div key={i} className="bg-white rounded-xl p-4 border border-surface-200">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-semibold text-surface-900">{p.year}</span>
                      <span className="text-success font-semibold">{p.margin} margin</span>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-center text-sm">
                      <div><div className="font-medium text-surface-900">{p.revenue}</div><div className="text-xs text-surface-800">Revenue</div></div>
                      <div><div className="font-medium text-surface-900">{p.expenses}</div><div className="text-xs text-surface-800">Costs</div></div>
                      <div><div className="font-medium text-success">{p.profit}</div><div className="text-xs text-surface-800">Profit</div></div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-gradient-to-r from-primary-500/5 to-accent-500/5 rounded-2xl border border-primary-500/20 p-8">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="bg-white/80 rounded-xl p-6 border border-surface-200">
              ...
              <div className="bg-white/80 rounded-xl p-6 border border-surface-200">
                <h3 className="font-semibold text-lg text-surface-800 mb-4">Traditional Agency (10 people)</h3>
                <ul className="space-y-3">
