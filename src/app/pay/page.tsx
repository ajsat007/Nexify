      ...
      <section className="relative pt-32 pb-20 gradient-bg overflow-hidden">
        <div className="section-container relative">
          <div className="max-w-3xl reveal">
            ...
            <p className="text-xl text-surface-400 max-w-2xl">Powered by Razorpay Live. Accept UPI, cards, net banking directly to your bank.</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-white dark:bg-surface-950">
        <div className="section-container">
          <div className="flex gap-2 mb-8 reveal">
            <button onClick={() => setTab('pay')} className={'px-6 py-3 rounded-xl text-sm font-medium transition-all ' + (tab === 'pay' ? 'bg-primary-500 text-white shadow-lg' : 'bg-surface-100 dark:bg-surface-800 text-surface-800')}>Pay Now</button>
            <button onClick={() => setTab('invoices')} className={'px-6 py-3 rounded-xl text-sm font-medium transition-all ' + (tab === 'invoices' ? 'bg-primary-500 text-white shadow-lg' : 'bg-surface-100 dark:bg-surface-800 text-surface-800')}>Invoices</button>
          </div>
          ...
          {tab === 'pay' && (
            <div className="grid lg:grid-cols-5 gap-8 reveal">
              <div className="lg:col-span-3 space-y-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4">
                    {amounts.map(a => (
                      <button key={a} onClick={() => { setAmount(a); setCustom('') }}
                        className={'p-4 rounded-xl border-2 text-center transition-all ' + (amount === a && !custom ? 'border-primary-500 bg-primary-50 dark:bg-primary-500/10 text-primary-600' : 'border-surface-200 dark:border-surface-700 text-surface-800 dark:text-surface-300 hover:border-surface-300')}>
                        <div className="text-lg font-heading font-bold">{a.toLocaleString('en-IN')}</div>
                        <div className="text-xs text-surface-800">{a >= 75000 ? 'Enterprise' : a >= 45000 ? 'Standard' : 'Starter'}</div>
                      </button>
                    ))}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-surface-800 dark:text-surface-300 mb-1">Custom Amount</label>
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-surface-800 font-medium"></span>
                ...
                <div className="card-surface p-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      <div key={m.id} className="relative p-4 rounded-xl border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/50">
                        ...
                        <div className="text-xs text-surface-800">{m.desc}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center gap-4 text-xs text-surface-800">
              ...
              <div className="lg:col-span-2">
                <div className="card-surface p-6 sticky top-28 space-y-6">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between"><span className="text-surface-800">Service Fee</span><span className="font-medium">{fa.toLocaleString('en-IN')}</span></div>
                    <div className="flex justify-between"><span className="text-surface-800">GST (18%)</span><span className="font-medium">{Math.round(fa * 0.18).toLocaleString('en-IN')}</span></div>
                  ...
                  <div className="bg-surface-50 dark:bg-surface-800/50 rounded-xl p-4 space-y-2 text-xs text-surface-800">
                    {[
                      '🔒 Secured by Razorpay',
                  ...
                  <div className="text-center pt-4 border-t border-surface-200 dark:border-surface-700">
                    <p className="text-xs text-surface-800 mb-2">Paid to</p>
                    <div className="font-semibold text-sm">Ajinkya Satkar</div>
                    <p className="text-xs text-surface-800">Pune, Maharashtra, India</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {tab === 'invoices' && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={i} className="card-surface p-5 text-center">
                    ...
                    <div className="text-xs text-surface-800 mt-1">{s.l}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3">
                {invoices.length === 0 && (
                  <div className="card-surface p-12 text-center text-surface-800">No invoices yet. Make your first payment!</div>
                )}
                  ...
                  <div key={inv.id} className="card-surface p-5 hover:shadow-lg transition-all">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="text-xs text-surface-800 mt-0.5">{inv.client} · {inv.date}</div>
                        {inv.paymentId && <div className="text-xs text-surface-800 mt-0.5">Payment ID: {inv.paymentId}</div>}
                      </div>
                      <div className="text-right">
                        <div className="text-lg font-heading font-bold">{inv.amount.toLocaleString('en-IN')}</div>
                        {inv.method && <div className="text-xs text-surface-800">{inv.method}</div>}
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="bg-gradient-to-r from-success/5 to-primary-500/5 rounded-2xl border border-success/20 p-6 text-center">
                ...
                <p className="text-sm text-surface-800 mb-4">Money settles directly to your bank account via Razorpay.</p>
