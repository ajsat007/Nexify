      ...
      <section className="section-padding bg-white">
        <div className="section-container">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-16 reveal">
            {stats.map((s, i) => (
              <div key={i} className="text-center"><div className="text-3xl font-heading font-bold gradient-text">{s.value}</div><div className="text-sm text-neutral-800 mt-1">{s.label}</div></div>
            ))}
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 reveal">
            {testimonials.map((t, i) => (
              <div key={i} className="bg-neutral-50 rounded-2xl p-6 border border-neutral-200 hover:border-primary-500/20 transition-all">
                ...
                <p className="text-neutral-800 text-sm leading-relaxed mb-6">&ldquo;{t.content}&rdquo;</p>
                <div className="flex items-center gap-3">
                  ...
                  <div><div className="font-semibold text-sm text-neutral-900">{t.name}</div><div className="text-xs text-neutral-800">{t.role}</div></div>
                </div>
              </div>
            ))}
          </div>
