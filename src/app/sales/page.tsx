...
function ProgressBar({ value, className = '' }: { value: number; className?: string }) {
  ...
  return (
    <div className={`w-full bg-neutral-100 rounded-full h-2 ${className}`}>
    ...
    <div className="bg-white rounded-xl p-5 border border-neutral-200 hover:shadow-md transition-all">
      <div className="flex items-center justify-between mb-3">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${variant === 'primary' ? 'bg-primary-50' : variant === 'success' ? 'bg-success/10' : variant === 'warning' ? 'bg-warning/10' : variant === 'accent' ? 'bg-accent-50' : 'bg-neutral-100'}`}>
          <Icon className={`w-5 h-5 ${variant === 'primary' ? 'text-primary-500' : variant === 'success' ? 'text-success' : variant === 'warning' ? 'text-warning' : variant === 'accent' ? 'text-accent-500' : 'text-neutral-500'}`} />
        </div>
  ...
  return (
    <div className="min-h-screen bg-neutral-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md text-center">
        ...
        <p className="text-neutral-400 mt-2 mb-8">AI-Powered Sales System</p>
        <div className="bg-neutral-800 rounded-2xl p-8 border border-neutral-700">
          ...
          <div className="space-y-4">
            <div><label className="block text-sm font-medium text-neutral-300 mb-1.5">Agent ID</label><input defaultValue="SalesAgent-Mu" className="w-full px-4 py-3 rounded-xl bg-neutral-700 border border-neutral-600 text-white focus:outline-none focus:ring-2 focus:ring-primary-500" /></div>
            <div><label className="block text-sm font-medium text-neutral-300 mb-1.5">Access Key</label><input type="password" defaultValue="••••••••" className="w-full px-4 py-3 rounded-xl bg-neutral-700 border border-neutral-600 text-white focus:outline-none focus:ring-2 focus:ring-primary-500" /></div>
          ...
          <p className="text-neutral-500 text-xs mt-4">Demo: click to enter</p>
        </div>
      </div>
    </div>
  )
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-400 text-sm">10 active deals · AI-powered lead scoring</p>
        </div>
      ...
      {showAdd && (
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          <div className="flex items-center justify-between mb-4"><h2 className="font-semibold">New Deal</h2><button onClick={() => setShowAdd(false)}><X size={18} className="text-neutral-400" /></button></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 overflow-x-auto">
        {stages.map(stage => {
          ...
          return (
            <div key={stage.id} className="bg-neutral-50 rounded-xl border border-neutral-200 min-w-[250px]">
              <div className="p-4 border-b border-neutral-200">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-sm text-neutral-900">{stage.label}</h3>
                  <span className="text-xs text-neutral-400">{stageDeals.length} deals</span>
                </div>
                <div className="text-lg font-heading font-bold text-neutral-900 mt-1">₹{(stageValue/100000).toFixed(1)}L</div>
              </div>
              <div className="p-3 space-y-3 min-h-[200px]">
                {stageDeals.map(d => (
                  <div key={d.id} className="bg-white rounded-xl p-4 border border-neutral-200 shadow-sm hover:shadow-md transition-all cursor-grab">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <div className="font-semibold text-sm text-neutral-900">{d.company}</div>
                        <div className="text-xs text-neutral-400">{d.contact}</div>
                      </div>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-neutral-400 shrink-0">{d.probability}%</span>
                    </div>
                    <div className="flex items-center justify-between text-xs text-neutral-400">
                      ...
                      <div className="flex gap-1">
                        ...
                        <button onClick={() => moveStage(d.id, 'forward')} disabled={stage.id === 'negotiation'} className="p-1 rounded hover:bg-neutral-100 disabled:opacity-30" title="Move forward"><ChevronUp size={14} /></button>
                      </div>
                    </div>
                  </div>
                ))}
                {stageDeals.length === 0 && <div className="text-center py-8 text-xs text-neutral-400">No deals in this stage</div>}
              </div>
            </div>
          )
        })}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-400 text-sm">AI-generated proposals in seconds</p>
        </div>
      ...
      <div className="space-y-3">
        {proposals.map(p => (
          <div key={p.id} className="bg-white rounded-xl border border-neutral-200 p-5 hover:shadow-lg transition-all">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-neutral-400">{p.company} · {p.type}</div>
                  <div className="text-xs text-neutral-400">Validity: {p.validUntil} · Sent: {p.sentDate}</div>
                </div>
              </div>
      ...
      <div className="bg-gradient-to-r from-primary-500/5 to-accent-500/5 rounded-2xl border border-primary-500/20 p-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500 mb-4">Our AI agent analyzes the lead's requirements, company profile, and industry to generate a personalized proposal with scope, timeline, pricing, and terms.</p>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-400 text-sm">AI-driven email campaigns · 0 cost (self-hosted SMTP)</p>
        </div>
      ...
      {showNewCamp && (
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div><label className="block text-xs font-medium text-neutral-700 mb-1">Campaign Name</label><input className="input-field text-sm" placeholder="Q3 Outreach" value={campForm.name} onChange={e => setCampForm(s => ({...s, name: e.target.value}))} /></div>
            <div><label className="block text-xs font-medium text-neutral-700 mb-1">Type</label><select className="input-field text-sm" value={campForm.type} onChange={e => setCampForm(s => ({...s, type: e.target.value}))}><option value="automated">Automated</option><option value="sequence">Sequence</option><option value="manual">Manual</option></select></div>
          </div>
      ...
      <div className="grid gap-4">
        {campaigns.map(c => (
          <div key={c.id} className="bg-white rounded-xl border border-neutral-200 p-5 hover:shadow-lg transition-all">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${c.status === 'active' ? 'bg-success/10' : 'bg-neutral-100'}`}>
                  {c.type === 'sequence' ? <RefreshCw className={`w-5 h-5 ${c.status === 'active' ? 'text-success' : 'text-neutral-400'}`} /> : <Send className={`w-5 h-5 ${c.status === 'active' ? 'text-success' : 'text-neutral-400'}`} />}
                </div>
                <div>
                  <div className="flex items-center gap-2"><span className="font-semibold text-sm text-neutral-900">{c.name}</span><Badge status={c.status} /><span className="text-xs text-neutral-400 capitalize">{c.type}</span></div>
                  <div className="text-xs text-neutral-400">Sent: {c.sent} · Opened: {c.opened} · Clicked: {c.clicked} · Replied: {c.replied}</div>
                </div>
              </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-400 text-sm">AI-powered WhatsApp Business API · 0 cost</p>
        </div>
      ...
      {showSend && (
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div><label className="block text-xs font-medium text-neutral-700 mb-1">Contact Name</label><input className="input-field text-sm" value={msgForm.contact} onChange={e => setMsgForm(s => ({...s, contact: e.target.value}))} /></div>
            <div><label className="block text-xs font-medium text-neutral-700 mb-1">Company</label><input className="input-field text-sm" value={msgForm.company} onChange={e => setMsgForm(s => ({...s, company: e.target.value}))} /></div>
          </div>
          <div className="mt-4">
      ...
      <div className="space-y-3">
        {messages.map(m => (
          <div key={m.id} className={`bg-white rounded-xl border p-4 hover:shadow-md transition-all ${m.direction === 'inbound' ? 'border-primary-200 bg-primary-50/30' : 'border-neutral-200'}`}>
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2"><span className="font-semibold text-sm text-neutral-900">{m.contact}</span><span className="text-xs text-neutral-400">{m.company}</span></div>
              ...
              <div className="text-xs text-neutral-400 text-right">{m.date} · {m.time}</div>
            </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-400 text-sm">AI agent automatically schedules and prioritizes follow-ups</p>
        </div>
      ...
      {showNew && (
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div className="sm:col-span-2"><label className="block text-xs font-medium text-neutral-700 mb-1">Action</label><input className="input-field text-sm" value={form.action} onChange={e => setForm(s => ({...s, action: e.target.value}))} /></div>
            <div><label className="block text-xs font-medium text-neutral-700 mb-1">Due Date</label><input type="date" className="input-field text-sm" value={form.dueDate} onChange={e => setForm(s => ({...s, dueDate: e.target.value}))} /></div>
            <div><label className="block text-xs font-medium text-neutral-700 mb-1">Priority</label><select className="input-field text-sm" value={form.priority} onChange={e => setForm(s => ({...s, priority: e.target.value}))}><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select></div>
          </div>
      ...
      <div className="space-y-2">
        {followups.map(f => {
          ...
          return (
            <div key={f.id} className={`bg-white rounded-xl border p-4 hover:shadow-md transition-all ${f.status === 'done' ? 'border-success/20 bg-success/5' : 'border-neutral-200'}`}>
              <div className="flex items-center gap-4">
                <button onClick={() => toggleDone(f.id)} className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${f.status === 'done' ? 'bg-success border-success' : 'border-neutral-300 hover:border-primary-500'}`}>
                  {f.status === 'done' && <CheckCircle2 size={14} className="text-white" />}
                </button>
                <div className="flex-1">
                  <div className={`text-sm font-medium ${f.status === 'done' ? 'text-neutral-400 line-through' : 'text-neutral-900'}`}>{f.action}</div>
      ...
      <div className="bg-gradient-to-r from-accent-500/5 to-primary-500/5 rounded-2xl border border-accent-500/20 p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-400">SalesAgent-Mu automatically schedules optimal follow-up times based on lead engagement patterns. 8 tasks auto-scheduled for today.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-400 text-sm">AI agents scan 7+ channels · Auto-qualify leads</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {sourceData.filter(s => s.count > 0).sort((a, b) => b.count - a.count).map(s => (
            <div key={s.source} className="flex items-center gap-4">
              ...
              <span className="text-xs text-neutral-400 w-20 text-right">₹{(s.value/100000).toFixed(1)}L</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-400 text-sm">AI-powered sales metrics · Real-time sync</p>
        </div>
        <div className="flex items-center gap-2 text-xs text-neutral-400 bg-neutral-100 rounded-xl px-3 py-1.5">
            ...
            {[
              ...
              return (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-semibold text-neutral-900 w-8">{s.value}</span>
                </div>
              )
            })}
          </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-start gap-3 text-sm">
                ...
                <div className="flex-1"><span className="text-neutral-500">{a.action}</span><div className="text-xs text-neutral-400">{a.time}</div></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-success/10 to-primary-500/10 rounded-2xl border border-success/20 p-5">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-neutral-500">AI sales agent is currently: Sending follow-up emails · Qualifying 2 new leads · Monitoring LinkedIn for signals</p>
          </div>
      ...
      <div className="grid grid-cols-3 gap-4 text-center">
        <div className="bg-white rounded-xl border border-neutral-200 p-4">
          ...
          <div className="text-xs text-neutral-400 mt-1">Pending Follow-ups</div>
        </div>
        <div className="bg-white rounded-xl border border-neutral-200 p-4">
          <div className="text-2xl font-heading font-bold gradient-text">{campaigns.filter(c => c.status === 'active').length}</div>
          <div className="text-xs text-neutral-400 mt-1">Active Campaigns</div>
        </div>
        <div className="bg-white rounded-xl border border-neutral-200 p-4">
          <div className="text-2xl font-heading font-bold gradient-text">₹0</div>
          <div className="text-xs text-neutral-400 mt-1">Sales Team Cost</div>
        </div>
      </div>
    </div>
  )
      ...
      <aside className={`fixed lg:sticky top-0 left-0 h-full w-64 bg-white border-r border-neutral-200 z-40 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-6 border-b border-neutral-100">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="text-neutral-900 font-heading font-bold text-sm">Nexify Sales</h2><p className="text-neutral-400 text-xs">AI Sales System</p></div>
          </div>
        </div>
        <nav className="p-4 space-y-1">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSidebarOpen(false) }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeTab === tab.id ? 'bg-primary-50 text-primary-600 border border-primary-200' : 'text-neutral-500 hover:bg-neutral-50 hover:text-neutral-700'}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-neutral-100">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex-1 min-w-0"><div className="text-sm font-medium text-neutral-900 truncate">SalesAgent-Mu</div><div className="text-xs text-neutral-400">Active</div></div>
            <Link href="/" className="text-neutral-400 hover:text-primary-500"><Home size={16} /></Link>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 min-w-0">
        <div className="sticky top-0 bg-white/80 backdrop-blur-xl border-b border-neutral-200 z-20">
          <div className="flex items-center justify-between px-4 sm:px-6 py-4">
            <button className="lg:hidden text-neutral-500" onClick={() => setSidebarOpen(true)}><Menu size={20} /></button>
            <div className="flex items-center gap-3 ml-auto">
              <Link href="/admin" className="text-xs text-neutral-400 hover:text-primary-500">Admin</Link>
              <Link href="/portal" className="text-xs text-neutral-400 hover:text-primary-500">Portal</Link>
            </div>
          </div>
        </div>
