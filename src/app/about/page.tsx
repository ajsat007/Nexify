        ...
        <div className="section-container">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-start">
            <div>
              ...
              <div className="space-y-4 text-surface-700 dark:text-surface-400 leading-relaxed text-sm sm:text-base">
                <p><strong className="text-surface-900 dark:text-white">The problem:</strong> Most software agencies treat clients like order-takers. Requirements → quote → build → deliver → ghost. No strategic partnership. No velocity. No transparency.</p>
                <p><strong className="text-surface-900 dark:text-white">The insight:</strong> What if an agency ran like a product company? What if it had its own SaaS products, its own AI agents, its own internal tools? What if it ate its own dog food before serving it to clients?</p>
                <p><strong className="text-surface-900 dark:text-white">The result:</strong> Nexify doesn&apos;t just build software. We run on the same systems we build for clients. Every process is battle-tested internally first.</p>
                <p><strong className="text-surface-900 dark:text-white">Today:</strong> 50+ AI agents serving clients across 12 countries, with 5 internal SaaS products generating recurring revenue.</p>
              </div>
            </div>
            <div className="bg-surface-50 dark:bg-surface-800 rounded-2xl p-6 sm:p-8 border border-surface-200 dark:border-surface-700">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-xl gradient-bg flex items-center justify-center">
                  <Cpu className="w-5 h-5 text-white" />
                </div>
                <div>
                  <div className="font-bold text-surface-900 dark:text-white text-sm">Nexify Technologies</div>
                  <div className="text-xs text-surface-600">AI-Native Software Company</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={s.label} className="bg-white dark:bg-surface-700/50 rounded-xl p-3 text-center">
                    <div className="font-heading font-bold text-lg sm:text-xl text-primary-600 dark:text-primary-400">{s.value}</div>
                    <div className="text-xs text-surface-700 dark:text-surface-300">{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="section-padding bg-surface-50 dark:bg-surface-900">
        <div className="section-container">
          <div className="text-center max-w-3xl mx-auto mb-10 sm:mb-16">
            ...
            <p className="text-surface-700 dark:text-surface-400 text-sm sm:text-base">The principles that guide every AI agent and every decision we make.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {values.map((v, i) => (
              <AnimatedSection key={v.title} animation="fade-up" delay={i * 50}>
                <div className="flex gap-4 bg-white dark:bg-surface-800 rounded-2xl p-5 sm:p-6 border border-surface-200 dark:border-surface-700 hover:border-primary-500/20 transition-all h-full">
                  ...
                  <div className="min-w-0">
                    <h4 className="font-semibold text-surface-900 dark:text-white mb-1 text-sm">{v.title}</h4>
                    <p className="text-surface-700 dark:text-surface-400 text-sm leading-relaxed">{v.desc}</p>
                  </div>
                </div>
              </AnimatedSection>
            ))}
