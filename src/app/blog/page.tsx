            ...
            <div className="flex flex-wrap gap-2">
              {categories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  className={`chip transition-all ${activeCategory === cat ? 'bg-primary-500 text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'}`}>{cat}</button>
              ))}
            </div>
            <button onClick={() => setShowGenerator(!showGenerator)}
              className={`chip transition-all ${showGenerator ? 'bg-accent-500 text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'}`}>
          ...
          {showGenerator && (
            <div className="mb-12 bg-gradient-to-r from-accent-500/5 to-primary-500/5 rounded-2xl border border-accent-500/20 p-6 reveal">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div><h2 className="font-semibold">AI Blog Post Generator</h2><p className="text-xs text-neutral-600">Powered by Nexify AI agents — generates in seconds</p></div>
              </div>
              <div className="flex flex-wrap gap-3 mb-4">
                {Object.keys(blogTemplates).map(topic => (
                  <button key={topic} onClick={() => setSelectedTopic(topic)}
                    className={`chip transition-all ${selectedTopic === topic ? 'bg-primary-500 text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'}`}>{topic}</button>
                ))}
              </div>
              ...
              {generatedPost && (
                <div className="mt-6 bg-white rounded-xl border border-neutral-200 p-6 animate-slide-up">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <button onClick={copyPost} className="shrink-0 px-3 py-1.5 rounded-lg bg-neutral-100 text-neutral-600 hover:bg-neutral-200 transition-all text-xs flex items-center gap-1">
                      {copied ? <><Check size={12} /> Copied</> : <><Copy size={12} /> Copy</>}</button>
                  </div>
                  <div className="prose prose-sm max-w-none text-neutral-600 whitespace-pre-line text-sm leading-relaxed">
                    {generatedPost.content}
                  </div>
                  <div className="flex items-center gap-2 mt-4 pt-4 border-t border-neutral-100 text-xs text-neutral-600">
              ...
              <article key={i} className="bg-white rounded-2xl border border-neutral-200 overflow-hidden hover:shadow-xl hover:border-primary-500/20 transition-all duration-300 group">
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="text-xs text-neutral-600 flex items-center gap-1"><Calendar size={12} />{post.date}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs text-neutral-600 pt-4 border-t border-neutral-100">
