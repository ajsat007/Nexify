...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-surface-950 text-white pt-24">
      <div className="section-container py-8">
        <div className="flex items-center gap-3 mb-8">
          <Bot className="w-8 h-8 text-primary-400" />
          <div><h1 className="text-2xl font-heading font-bold">AI Operations Dashboard</h1><p className="text-surface-500 text-sm">{data.counts.total} AI Employees · {data.counts.byStatus.active} Active</p></div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            const Icon = s.icon
            return <div key={s.label} className="bg-surface-800/50 border border-surface-700 rounded-2xl p-5"><div className="flex items-center justify-between mb-2"><Icon className={`w-5 h-5 ${s.color}`} /><span className="text-2xl font-heading font-bold">{s.value}</span></div><div className="text-surface-500 text-sm">{s.label}</div></div>
          })}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-8">
          {Object.entries(data.counts.byDepartment).map(([dept, count]) => (
            <div key={dept} className="bg-surface-800/30 border border-surface-700/50 rounded-xl px-4 py-3 flex items-center justify-between">
              <span className="text-sm capitalize text-surface-400">{dept}</span>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {data.agents.map((agent: any) => (
            <div key={agent.id} className="bg-surface-800/30 border border-surface-700/50 rounded-2xl p-4 hover:border-primary-500/30 transition-all">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-surface-600">{agent.department} · {agent.status}</div>
                </div>
              </div>
              <p className="text-xs text-surface-500 mb-2 line-clamp-2">{agent.title}</p>
              <div className="flex flex-wrap gap-1">
                {agent.capabilities.slice(0, 3).map((c: string) => (
                  <span key={c} className="text-xs px-2 py-0.5 rounded-full bg-surface-700/50 text-surface-400">{c}</span>
                ))}
              </div>
            </div>
          ))}
          ...
          <>
            ...
            <div className="space-y-3">
              {data.workflows.map((wf: any) => (
                <div key={wf.id} className="bg-surface-800/30 border border-surface-700/50 rounded-xl px-5 py-4 flex items-center justify-between">
                  <div>
                    <div className="text-sm font-semibold">{wf.name}</div>
                    <div className="text-xs text-surface-500">{wf.steps} steps · {new Date(wf.createdAt).toLocaleString()}</div>
                  </div>
